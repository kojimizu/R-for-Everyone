...
$$
OD = \frac{1}{n-p}\Sigma_{i=1}^n z_i^2 \\
z_i=\frac{y_i-\hat{y_i}}{sd(hat{y_i)}
$$
...
As a measure, we use residual analysis.
-dataset: housing
```{r}
...
ggplot(data=housing,aes(x=Units,y=ValuePerSqFt))+geom_point()
```
...
After data setting, we will fit a linear model,taking valuePerSqFt as object variable.
```{r}
# Model estimation
